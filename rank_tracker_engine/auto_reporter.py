...
def send_telegram(token, chat_id, message):
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"Telegram API 거부(에러): {resp.text}")
...
async def main():
    ...
    with open(targets_path, "r", encoding="utf-8", errors="ignore") as f:
        targets = json.load(f)
    ...
    # 3. 프록시 정보 불러오기
    proxy_config_path = os.path.join(os.path.dirname(__file__), "proxy_config.txt")
    if not os.path.exists(proxy_config_path):
        print("proxy_config.txt 파일이 없습니다. (깃허브에 업로드 안 됨)")
        sys.exit(1)
    ...
    print(f"\n[알림 전송 중... 준비된 메시지 길이: {len(final_message)}자]")
